...
class Beta(Distribution):
    ...
    def tf_sample(self, distr_params, deterministic):
        ...
        return self.min_value + tf.where(condition=deterministic, x=definite, y=sampled) * (self.max_value - self.min_value)
    ...
    def tf_log_probability(self, distr_params, action):
        ...
        return (beta - 1.0) * tf.log(x=tf.maximum(x=action, y=util.epsilon)) + (alpha - 1.0) * tf.log1p(x=-action) - log_norm

    def tf_entropy(self, distr_params):
        alpha, beta, alpha_beta, log_norm = distr_params
        return log_norm - (beta - 1.0) * tf.digamma(x=beta) - (alpha - 1.0) * tf.digamma(x=alpha) + (alpha_beta - 2.0) * tf.digamma(x=alpha_beta)
    ...
    def tf_kl_divergence(self, distr_params1, distr_params2):
        ...
        return log_norm2 - log_norm1 - tf.digamma(x=beta1) * (beta2 - beta1) - tf.digamma(x=alpha1) * (alpha2 - alpha1) + tf.digamma(x=alpha_beta1) * (alpha_beta2 - alpha_beta1)
