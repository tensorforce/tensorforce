...
class Conv1d(TransformationBase):
    ...
    def output_spec(self):
        ...
        if self.squeeze:
            output_spec['shape'] = shape
        else:
            output_spec['shape'] = shape + (self.size,)
...
class Conv2d(TransformationBase):
    ...
    def output_spec(self):
        ...
        height = conv_output_length(
            input_length=output_spec['shape'][0], filter_size=self.window[0],
            padding=self.padding, stride=self.stride[1], dilation=self.dilation[1]
        )
        width = conv_output_length(
            input_length=output_spec['shape'][1], filter_size=self.window[1],
            padding=self.padding, stride=self.stride[2], dilation=self.dilation[2]
        )
        shape = (height, width)
