        ...
        def function(name, spec, advantage_layer):
            ...
            if spec.type == 'bool':
                ...
                def fn_summary():
                    ...
                    mean_adv = tf.math.reduce_mean(
                        input_tensor=advantage_value, axis=-1, keepdims=True
                    )
                    action_value = state_value + (advantage_value - mean_adv)
                ...
                def fn_summary():
                    shape = (-1,) + (spec.size,) + (spec.num_values,)
                    adv_value = tf.reshape(tensor=advantage_value, shape=shape)
                    state_value = value_layer.apply(x=embedding)
                    state_value = tf.expand_dims(input=state_value, axis=-1)
                    mean_adv = tf.math.reduce_mean(
                        input_tensor=adv_value, axis=-1, keepdims=True
                    )
                    action_value = state_value + (adv_value - mean_adv)
                ...
                with tf.control_dependencies(control_inputs=dependencies):
                    ...
                    return tf.math.argmax(
                        input=advantage_value, axis=-1, output_type=spec.tf_type()
                    )
