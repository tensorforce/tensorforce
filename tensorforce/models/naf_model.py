...
class NAFModel(Model):
    ...
    def create_tf_operations(self, config):
        ...
        with tf.variable_scope('training_outputs'):
            ...
            advantage = -0.5 * tf.matmul(tf.expand_dims(action_diff, 1), tf.matmul(p_matrix, tf.expand_dims(action_diff, 2)))
        ...
        with tf.name_scope("update"):
            for action in self.action:
                ...
                loss = tf.reduce_mean(tf.squared_difference(q_target, q_value[:-1]), name='loss')
    ...
    # def create_naf_outputs(self, outputs, actions):
    ...
    #
    #     """
    #     num_actions = actions.get_shape()[1].value
    #     # State-value function
    #     value = layers['linear'](x=outputs, size=1)
    #     # Action outputs
    #     mean = layers['linear'](x=outputs, size=num_actions)
    #
    #     # Advantage computation
    #     # Network outputs entries of lower triangular matrix L
    #     lower_triangular_size = int(num_actions * (num_actions + 1) / 2)
    #     l_entries = layers['linear'](x=outputs, size=lower_triangular_size)
    #
    #     # Iteratively construct matrix. Extra verbose comment here
    #     l_matrix = tf.exp(tf.diag(diagonal=l_entries[:num_actions]))
    #     offset = num_actions
    #     for n, size in enumerate(xrange(num_actions - 1, -1, -1)):
    #         l_matrix[:, n, n + 1:] = l_entries[offset: offset + size]
    #         offset += size
    #     # P = LL^T
    #     p_matrix = tf.matmul(l_matrix, tf.transpose(l_matrix, (0, 2, 1)))
    #
    ...
    #     # l_matrix = tf.transpose(tf.stack(l_rows, axis=1), (0, 2, 1))
    #
    #     action_diff = tf.expand_dims(actions - mean, -1)
    #     # A = -0.5 (a - mean)P(a - mean)
    #     advantage = -0.5 * tf.matmul(tf.transpose(action_diff, (0, 2, 1)), tf.matmul(p_matrix, action_diff))
